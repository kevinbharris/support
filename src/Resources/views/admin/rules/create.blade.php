        ...
        <form action="{{ route('admin.support.rules.store') }}" method="POST" class="bg-white dark:bg-gray-900 rounded box-shadow">
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" class="flex w-full min-h-[75px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="from_status_id" id="from_status_id" class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900" required>
                        ...
                        @foreach($statuses as $status)
                            <option value="{{ $status->id }}" {{ old('from_status_id') == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="to_status_id" id="to_status_id" class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900" required>
                        <option value="">@lang('Select Status')</option>
                        @foreach($statuses as $status)
                            <option value="{{ $status->id }}" {{ old('to_status_id') == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                        @endforeach
                    </select>
                    @error('to_status_id')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="after_hours" id="after_hours" class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900" value="{{ old('after_hours', 24) }}" min="1" required>
                ...
                <div>
                    <label class="flex items-center text-sm text-gray-600 dark:text-gray-300">
                        <input type="checkbox" name="is_enabled" value="1" {{ old('is_enabled', true) ? 'checked' : '' }} class="mr-2">
                        @lang('Enabled')
                    </label>
                </div>
            </div>
