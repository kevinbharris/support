        ...
        <form action="{{ route('admin.support.tickets.update', $ticket->id) }}" method="POST" class="bg-white dark:bg-gray-900 rounded box-shadow">
            ...
            <div class="p-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="assigned_to" id="assigned_to" class="flex w-full min-h-[39px] py-2 px-3 border rounded-md text-sm text-gray-600 dark:text-gray-300 transition-all hover:border-gray-400 focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900" value="{{ old('assigned_to', $ticket->assigned_to) }}">
                    @error('assigned_to')
                        <span class="text-xs text-red-600">{{ $message }}</span>
                    @enderror
                </div>
